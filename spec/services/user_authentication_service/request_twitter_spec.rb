describe UserAuthenticationService::RequestTwitter do
  ...
  describe '#provider' do
    it { expect(subject.provider).to eq('twitter') }
  end
  ...
  describe 'dependents from request_auth' do
    ...
    # TODO Find a more accurate method for this. Because this is set to Time.now + 31540000
    # which varies from calling Time.now + 31540000 later in the test. So it is
    # currently rounded to the nearest 5 seconds.
    describe '#credentials_expiration' do
      it do
        received = (subject.credentials_expiration.to_i / 5).round * 5
        expected = ((Time.now.to_i + 31540000) / 5).round * 5
  ...
  describe '#valid?' do
    ...
    before do
      ...
      ENV['TWITTER_CLIENT_ID'] = 'dUmmy_r3qu3st_t0k3n'
    end
